...
@Transactional
public void updateDrugsWithPDF(String batchId, String s3RelativePath) {
    System.out.println("DEBUG: batchId in DrugDao = " + batchId);
    System.out.println("DEBUG: S3 Relative Path to be saved = " + s3RelativePath);

    try {
        Query query = entityManager.createQuery(
            "UPDATE Drug d SET d.pdfFileName = :s3RelativePath WHERE d.prescriptionBatchId = :batchId"
        );
        query.setParameter("s3RelativePath", s3RelativePath);
        query.setParameter("batchId", batchId);

        int updatedRecords = query.executeUpdate();

        if (updatedRecords > 0) {
            System.out.println("PDF Relative Path updated in database: " + s3RelativePath + " for batch ID: " + batchId);
        } else {
            System.out.println("⚠ No records updated. Check batch ID: " + batchId);
        }
    } catch (Exception e) {
        System.err.println("Error updating PDF Relative Path: " + e.getMessage());
        e.printStackTrace();
        throw e;  // Ensure rollback in Spring
    }
}
