...
if (drug.getEndDate() != null) {
	...
	public static class Prescription {
		...
		public String getFullOutLine() {
			...
			if (getRoute() != null && !getRoute().isEmpty()) {
				extra += "Route: " + getRoute() + "; ";
			
				// Log to Tomcat console (catalina.out or IDE console)
				System.out.println("=== [DEBUG] Route added to full outline: " + getRoute());
			}
		    ...
		    if (getDrugForm() != null && !getDrugForm().trim().isEmpty()) {
				extra += "Form: " + getDrugForm() + "; ";
			}
	...
	if (getLongterm_p() != null) {
		String longTermValue = getLongterm_p() ? "Yes" : "No";
		extra += "; LongTerm:" + longTermValue;
		System.out.println("DEBUG: LongTerm field value in full outline is: " + longTermValue);
	}
	

String fullOutLine = RxPrescriptionData.getFullOutLine(getSpecial() + "\n" + extra);

// ✅ Print the full outline string
System.out.println("DEBUG: FullOutLine = " + fullOutLine);
