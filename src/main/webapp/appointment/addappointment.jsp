...
<% } %>
  ...
  let html = "<div><b>" + item.label + "</b>";
  if (item.provider && item.provider.trim() !== "null" && item.provider.trim() !== "") {
    html += "<br>" + item.provider;
  }
  html += "</div>";
    ...
    <td>
        <label style="font-size: 14px; margin-right: 10px;">
            <input type="radio" name="deliveryOption" value="Delivery"
                ...
                <%= "Delivery".equals(request.getParameter("resources")) ? "checked" : "" %>> Delivery
        </label>
        <label style="font-size: 14px;">
            <input type="radio" name="deliveryOption" value="Pickup"
                onclick="updateDeliveryOption(this.value)"
                <%= "Pickup".equals(request.getParameter("resources")) ? "checked" : "" %>> Pickup
        </label>

        <!-- Hidden field that gets submitted as "resources" -->
        <input type="hidden" id="resources" name="resources"
            value="<%= bFirstDisp ? "" : request.getParameter("resources") %>" />
    </td>
</tr>
<%
    } // end if ENTITY_TYPE is clinic
%>
...
<div class ="span12">
        ...
        <input type="submit" id="addButton" class="btn btn-primary"
            onclick="return validateModeSelection() && (document.forms['ADDAPPT'].displaymode.value='Add Appointment');"
            tabindex="6"
        ...
        <table style="font-size: 9pt; background-color:#e8e8e8; text-align:center; vertical-align: top; padding:3px;">
            <tr style="background-color:#e8e8e8;">
                <th colspan="2">
                    ...
                    <a title="Master File" onclick="popup(window.screen.height - 250, window.screen.width - 350,'<%=request.getContextPath() %>/demographic/demographiccontrol.jsp?demographic_no=<%=demoNo%>&amp;displaymode=edit&amp;dboperation=search_detail','master')" href="javascript: function myFunction() {return false; }"><bean:message key="appointment.addappointment.btnEdit"/></a>
...
<script type="text/javascript">
    function updateDeliveryOption(value) {
        document.getElementById("resources").value = value;
    }
</script>
...
function validateModeSelection() {
    if (entityType.toLowerCase() === "clinic") {
        var mode = document.getElementById("resources").value;
        if (!mode || mode.trim() === "") {
            alert("Please select a Mode: Delivery or Pickup.");
            return false;
        }
    }
    return true; // No validation needed for non-clinic
}
</script>
